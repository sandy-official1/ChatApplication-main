...
createGroupBtn.addEventListener("click", () => {
  ...
  axios
    .get("http://localhost:3000/UserDetail?", {
      headers: {
        Authorization: token,
      },
    })
...
createGroupSubmitBtn.addEventListener("click", async function (event) {
  ...
  const token = localStorage.getItem("token");
  const isCreateGroup = await axios.post(
...
async function myFunction(groupid, groupName) {
  ...
  const userchat = await axios.get(
    `http://localhost:3000/fetchGroupChat/?groupid=${groupid} & lastmessageid=${LastMessageId}`,
    {
      headers: {
        Authorization: token,
      },
  ...
  if (totalmesage > 0) {
    localStorage.setItem(
      "Lastmessageid",
      userchat.data.userchat[totalmesage - 1].id
    );
...
document.getElementById("send-btn").addEventListener("click", async (e) => {
  try {
    ...
    const respone = await axios.post(
      "http://localhost:3000/addGroupMessage",
      msg,
      {
        headers: {
          Authorization: token,
  ...
  for (let index = 0; index < message.data.userchat.length; index++) {
    if (message.data.userchat[index].tbluserdetailId === userid) {
      ...
      messageElem.textContent =
        message.data.userchat[index].message + "-" + "You";
      ...
      const messageElem = document.createElement("p");
      messageElem.textContent =
        message.data.userchat[index].tbluserdetail.username +
        "-" +
        message.data.userchat[index].message;
...
async function groupPermission(groupid, groupname) {
  try {
    ...
    const isadmin = await axios.get(
      `http://localhost:3000/checkAdmin?groupid=${groupid}`,
      {
        headers: {
          Authorization: token,
        },
    ...
    if (
      ...
      isadmin.data.isAdmin.isSuperAdmin === true
    ) {
